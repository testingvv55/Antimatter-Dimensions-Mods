...
function updateDimensions() {
    if (document.getElementById("antimatterdimensions").style.display == "block" && document.getElementById("dimensions").style.display == "block") {
        ...
        if (player.timestudy.studies.includes(225)) extraGals += Math.floor(player.replicanti.amount.logarithm / 1000)
    ...
    if (document.getElementById("infinity").style.display == "block") {
        if (document.getElementById("preinf").style.display == "block") {
            ...
            document.getElementById("infi32").innerHTML = "Multiplier for unspent Infinity Points on 1st Dimension<br>Currently: " + formatValue(player.options.notation, player.infinityPoints.dividedBy(2).max(0).pow(1.5).plus(1), 2, 2) + "x<br>Cost: 5 IP"
            ...
            document.getElementById("postinfi11").innerHTML = "Power up all dimensions based on total antimatter produced<br>Currently: "+ Math.pow(player.totalmoney.logarithm+1, 0.5).toFixed(2)+"x<br>Cost: "+shortenCosts(1e4)+" IP"
            document.getElementById("postinfi21").innerHTML = "Power up all dimensions based on current antimatter<br>Currently: "+ Math.pow(player.money.logarithm+1, 0.5).toFixed(2)+"x<br>Cost: "+shortenCosts(5e4)+" IP"
...
function updateInfCosts() {
    ...
    if (document.getElementById("timestudies").style.display == "block" && document.getElementById("eternitystore").style.display == "block") {
        document.getElementById("11desc").textContent = "Currently: "+shortenMoney(Decimal.fromMantissaExponent(10 -player.tickspeed.dividedBy(1000).pow(0.005).times(0.95).plus(player.tickspeed.dividedBy(1000).pow(0.0003).times(0.05)).mantissa, Math.abs(player.tickspeed.dividedBy(1000).pow(0.005).times(0.95).plus(player.tickspeed.dividedBy(1000).pow(0.0003).times(0.05)).logarithm)).min("1e2500").max(1))+"x"
        ...
        document.getElementById("51desc").textContent = "You gain "+shortenCosts(1e30)+"x more IP"
        ...
        document.getElementById("161desc").textContent = shortenCosts(new Decimal("1e3080"))+"x multiplier on all normal dimensions"
        document.getElementById("162desc").textContent = shortenCosts(1e55)+"x multiplier on all Infinity dimensions"
        ...
        if (player.etercreq !== 1) document.getElementById("ec1unl").innerHTML = "Eternity Challenge 1<span>Requirement: "+(ECTimesCompleted("eterc1")+1)*1000+" Eternities<span>Cost: 30 Time Theorems"
...
function gainedInfinityPoints() {
    ...
    var ret = Decimal.pow(10, player.money.logarithm/div -0.75).times(player.infMult)
...
function gainedEternityPoints() {
    var ret = Decimal.pow(5, player.infinityPoints.plus(gainedInfinityPoints()).logarithm/308 -0.7).times(player.epmult)
...
function calcSacrificeBoost() {
    ...
    if (player.currentChallenge != "challenge11") {
        ...
        return Decimal.pow((player.firstAmount.logarithm/10.0), sacrificePow).dividedBy(((Decimal.max(player.sacrificed.e, 1)).dividedBy(10.0)).pow(sacrificePow).max(1)).max(1);
    } else {
...
function calcTotalSacrificeBoost() {
    ...
    if (player.currentChallenge != "challenge11") {
        ...
        return Decimal.pow((player.sacrificed.logarithm/10.0), sacrificePow);
    } else {
...
function canUnlockEC(idx, cost, study, study2) {
    ...
    switch(idx) {
        case 1:
        if (player.eternities >= 1000+(ECTimesCompleted("eterc1")*1000)) return true
        break;
...
setInterval(function() {
    ...
    if (player.tickspeed.logarithm < -8296262) giveAchievement("Faster than a potato^286078")
    if (player.timestudy.studies.length == 0 && player.dilation.active && player.infinityPoints.logarithm >= 20000) giveAchievement("This is what I have to do to get rid of you.")
...
function gameLoop(diff) {
    ...
    if (player.infMultBuyer) {
        var dif = Math.floor(player.infinityPoints.ln()/Math.log(4) - player.infMultCost.ln()/Math.log(4)) +1
        if (dif > 0) {
            player.infMult = player.infMult.times(Decimal.pow(2, dif))
            player.infMultCost = player.infMultCost.times(Decimal.pow(4, dif))
            document.getElementById("infiMult").innerHTML = "Multiply infinity points from all sources by 2 <br>currently: "+shorten(player.infMult) +"x<br>Cost: "+formatValue(player.options.notation,player.infMultCost,3)+" IP"
            player.infinityPoints = player.infinityPoints.minus(player.infMultCost.dividedBy(4))
    ...
    if (player.currentChallenge !== "") {
        ...
        document.getElementById("progressbar").style.width = Decimal.min(player.money.logarithm / getNewInfReq().logarithm * 100, 100).toFixed(2) + "%"
        document.getElementById("progresspercent").textContent = Decimal.min(player.money.logarithm / getNewInfReq().logarithm * 100, 100).toFixed(2) + "%"
    ...
    // let extraGals = 0
    // if (player.timestudy.studies.includes(225)) extraGals += Math.floor(player.replicanti.amount.logarithm / 2500)
...
var ec10bonus = new Decimal(1)
function updatePow() {
    totalMult = Math.pow(player.totalmoney.logarithm+1, 0.5)
    currentMult = Math.pow(player.money.logarithm+1, 0.5)
    ...
    unspentBonus = player.infinityPoints.dividedBy(2).max(0).pow(1.5).plus(1)
